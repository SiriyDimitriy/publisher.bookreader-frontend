...
const BookNameContent = Content.extend`border-bottom: solid 0.2rem #dddddd;`;
...
export default class BookHeader extends React.Component {
  ...
  componentDidMount() {
    let bookNameHeight = getComputedStyle(ReactDOM.findDOMNode(this.bookName))
      .height;
    let bookAuthorNameHeight = getComputedStyle(
      ReactDOM.findDOMNode(this.bookAuthorName)
    ).height;
    this.setState({
      bookNameHeight,
  ...
  render() {
    ...
    console.log('bookName', bookName, bookAuthor, bookId);
    return (
      <Page>
        <ImageWrapper>
          {this.props.avatar ? (
